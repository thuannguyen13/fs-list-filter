...
const initConditionValueField = (
  ...
  for (const internalConditionValueFormField of internalConditionValueFormFields) {
    ...
    allConditionValueFormFields.set(type, [
      ...(allConditionValueFormFields.get(type) ?? []),
      internalConditionValueFormField,
    ]);
  }
  ...
  for (const externalConditionValueFormField of externalConditionValueFormFields) {
    ...
    allConditionValueFormFields.set(type, [...(allConditionValueFormFields.get(type) ?? []), clone]);
  }
  ...
  const changeHandler = () => {
    ...
    const activeFormField = allConditionValueFormFields.get(activeFormFieldType)![0];

    const fuzzyThreshold = getAttribute(activeFormField, 'fuzzy');
    const splitSeparator = getSplitSeparator(activeFormField);

    let value = getFormFieldValue(activeFormField, CUSTOM_VALUE_ATTRIBUTE);
  ...
  const changeCleanups = [...allConditionValueFormFields].flatMap(([, formFields]) =>
  ...
  const formFieldsHandler = ([fieldKey, op, allFieldsData]: [
    ...
    for (const [type, formFields] of allConditionValueFormFields) {
      if (type === activeFormFieldType) {
        let anchor: Element | Comment = conditionValueFieldAnchor;
        for (const formField of formFields) {
          anchor.after(formField);
          anchor = formField;
        }
      } else {
        for (const formField of formFields) {
    ...
    if (activeFormFieldType && previouslyActiveFormFieldType !== activeFormFieldType) {
      const activeFormFields = allConditionValueFormFields.get(activeFormFieldType)!;
      const activeFormField = activeFormFields[0];

      if (isHTMLSelectElement(activeFormField)) {
        activeFormField.selectedIndex = 0;
      } else if (activeFormField.type === 'radio' || activeFormField.type === 'checkbox') {
        setFormFieldValue(activeFormField, activeFormField.type === 'checkbox' ? [] : '', CUSTOM_VALUE_ATTRIBUTE);
      } else {
  ...
  const optionsHandler = ([fieldKey, allFieldsData]: [FiltersCondition['fieldKey'] | undefined, AllFieldsData]) => {
    const selectElements = [...allConditionValueFormFields.values()].flat().filter(isHTMLSelectElement);
    if (!selectElements.length) return;

    const activeSelect = activeFormFieldType
      ? allConditionValueFormFields.get(activeFormFieldType)?.[0]
      : undefined;
  ...
  const twoWayBindingCleanup = watch(
    ...
    (value: FiltersCondition['value'] | undefined, oldValue: FiltersCondition['value']) => {
      ...
      for (const formFields of allConditionValueFormFields.values()) {
        setFormFieldValue(formFields[0], value, CUSTOM_VALUE_ATTRIBUTE);
      }
    }
  );
...
export const initCondition = (list: List, element: HTMLElement, conditionGroup: ConditionGroup) => {
  ...
  const value = getFormFieldValue(initialConditionValueFormField, CUSTOM_VALUE_ATTRIBUTE);
